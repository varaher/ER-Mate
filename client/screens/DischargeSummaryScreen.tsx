...
export default function DischargeSummaryScreen() {
  ...
  useEffect(() => {
    if (caseData && !summaryRef.current.course_in_hospital && !autoGeneratedRef.current && !generating) {
  ...
  const generateCourseInHospital = async () => {
    ...
    try {
      ...
      const response = await fetch(endpoint.toString(), {
        ...
        body: JSON.stringify({
          case_id: caseId,
          // Full raw case — backend reads everything from here
          full_case: caseData,
          // Keep summary_data for backward compat (discharge vitals, doctor sigs, etc.)
          summary_data: {
            ...summaryRef.current,
            ...
            >
              {generating ? (
                ...
                <>
                  ...
                  <Text style={[styles.aiBtnText, { color: theme.primary }]}>
                    {summaryRef.current.course_in_hospital ? "Regenerate Course in Hospital" : "Generate Course in Hospital"}
                  </Text>
                </>
              )}
            </Pressable>
